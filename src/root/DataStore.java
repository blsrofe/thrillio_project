...
//                "http://tomcat.apache.org/tomcat-6.0-doc/virtual-hosting-howto.html", "http://tomcat.apache.org");
        ...
        for(String row : data) {
            String[] values = row.split(",");
            bookmarks[0][colNum++] = BookmarkManager.getInstance().createWebLink(Long.parseLong(values[0]), values[1], "profile url", values[2], values[3]);
        }
    }
...
//                "profile url", 2007, "Prentice Hall", new String[] { "Joshua Bloch" }, BookGenre.TECHNICAL, 4.9);
        ...
        for(String row : data) {
            ...
            bookmarks[2][colNum++] = BookmarkManager.getInstance().createBook(Long.parseLong(values[0]), values[1], "profile Url", Integer.parseInt(values[2]), values[3], authors, values[5], Double.parseDouble(values[6]));
        }
    }
...
//                MovieGenre.FOREIGN_MOVIES, 8.4);
        ...
        for(String row : data) {
            ...
            bookmarks[1][colNum++] = BookmarkManager.getInstance().createMovie(Long.parseLong(values[0]), values[1], "profile url", Integer.parseInt(values[2]), cast, directors, values[5], Double.parseDouble(values[6]));
        }
    }
